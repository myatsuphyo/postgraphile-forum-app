...
comment on column app_public.post.title is 'The title written by the user.';
...
create function app_public.person_name(person app_public.person) returns text as $$
  select person.first_name || ' ' || person.last_name
$$ language sql stable;

comment on function app_public.person_name(app_public.person) is 'A person’s full name which is a concatenation of their first and last name.';
...
create function app_public.search_posts(search text) returns setof app_public.post as $$
  select post.*
  from app_public.post as post
  where post.title ilike ('%' || search || '%') or post.body ilike ('%' || search || '%')
$$ language sql stable;
...
create function app_public.authenticate(
  ...
  select ('forum_example_person', person_id)::app_public.jwt_token
...
grant usage on schema app_public to forum_example_anonymous, forum_example_person;

grant select on table app_public.person to forum_example_anonymous, forum_example_person;
grant update, delete on table app_public.person to forum_example_person;

grant select on table app_public.post to forum_example_anonymous, forum_example_person;
grant insert, update, delete on table app_public.post to forum_example_person;
grant usage on sequence app_public.post_id_seq to forum_example_person;

grant execute on function app_public.person_name(app_public.person) to forum_example_anonymous, forum_example_person;
grant execute on function app_public.post_summary(app_public.post, integer, text) to forum_example_anonymous, forum_example_person;
grant execute on function app_public.person_latest_post(app_public.person) to forum_example_anonymous, forum_example_person;
grant execute on function app_public.search_posts(text) to forum_example_anonymous, forum_example_person;
grant execute on function app_public.authenticate(text, text) to forum_example_anonymous, forum_example_person;
grant execute on function app_public.current_person() to forum_example_anonymous, forum_example_person;
grant execute on function app_public.change_password(text, text) to forum_example_person;

grant execute on function app_public.register_person(text, text, text, text) to forum_example_anonymous;
...
create policy update_person on app_public.person for update to forum_example_person
  using (id = current_setting('jwt.claims.person_id', true)::integer);

create policy delete_person on app_public.person for delete to forum_example_person
  using (id = current_setting('jwt.claims.person_id', true)::integer);

create policy insert_post on app_public.post for insert to forum_example_person
  with check (author_id = current_setting('jwt.claims.person_id', true)::integer);

create policy update_post on app_public.post for update to forum_example_person
  using (author_id = current_setting('jwt.claims.person_id', true)::integer);

create policy delete_post on app_public.post for delete to forum_example_person
  using (author_id = current_setting('jwt.claims.person_id', true)::integer);
